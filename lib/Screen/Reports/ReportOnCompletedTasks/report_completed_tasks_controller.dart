...
import 'report_completed_tasks_model.dart';

class ReportOnCompletedTasksController extends GetxController with StateMixin<ReportOnCompletedTasksModel> {
      ...
      try {
        ...
        await ApiService().getData(urlgetPrfile, {'Authorization': 'Bearer ${stg.read(token)}'}).timeout(
          ...
          if (value.statusCode == 200) {
            print("startgetReportOnCompletedTasks");
            ReportOnCompletedTasksModel? modelReportOnCompletedTasks = ReportOnCompletedTasksModel.fromJson(value.data);
            print(value.data);
            change(modelReportOnCompletedTasks, status: RxStatus.success());
        ...
        print("EndgetReportOnCompletedTasks");
      } catch (e) {
        print(e.toString());
