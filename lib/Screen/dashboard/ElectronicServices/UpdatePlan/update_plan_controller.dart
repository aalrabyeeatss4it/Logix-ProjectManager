import 'dart:async';
import 'dart:convert';
import 'package:projectmanagers/Screen/dashboard/ElectronicServices/UpdatePlan/update_plan_model.dart';
...
class UpdatePlanController extends GetxController with StateMixin<UpdatePlanModel> {
      ...
      try {
        ...
        await ApiService().getData(urlgetPrfile, {'Authorization': 'Bearer ${stg.read(token)}'}).timeout(
          ...
          if (value.statusCode == 200) {
            print("startgetUpdatePlan");
            UpdatePlanModel? modelUpdatePlan = UpdatePlanModel.fromJson(value.data);
            print(value.data);
            change(modelUpdatePlan, status: RxStatus.success());
